...
int jack_callback (jack_nframes_t nframes, void *arg){
	...
	//Paso 0: copiar aux a primeras muestras de out
	for(i=0 ; i < d ; ++i,++j){
		out[i] = aux[i];
	}

	//Paso 1: copiar d muestras de in al final de out
	for(i = 0; i < nframes - d ; i++){
		out[i+d] = in[i];
	}
	//Paso 2: copiar resto de in a aux
	for(; i < nframes ; ++i,++j){
		aux[j] = in[i];
	}
...
int main (int argc, char *argv[]) {
	...
	aux = (jack_default_audio_sample_t)
