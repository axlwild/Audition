...
/*
		Ejercicio Desfase
		Dado un argumento numérico entero del usuario, hacer que la salida
		esté desfasada por igual número de frames.

		Hacer que se desface en tiempos más grande qu euna ventana de JACK
*/
...
int jack_callback (jack_nframes_t nframes, void *arg){
	...
	//----------------------------------------------------------------> Cambiar memcpy a for
	//memcpy (out, in, nframes * sizeof (jack_default_audio_sample_t));


	for (i = 0; i < nframes ; ++i)
	{
		//Primero copamos la entrada al auxiliar de tamaño d (delay)
		out[i] = aux[aux_i];
		//Luego modificamos el auxiliar 
		aux[aux_i] = in[i];
		aux_i ++ ;
...
int main (int argc, char *argv[]) {
	if (argc < 2){
		printf("Need a numeric number for seconds to delay\n");
		exit(1);
	}
	//Delay [seconds]
	double d_seg = atof(argv[1]);
	printf("d_seg %f\n", d_seg);
	//exit(0);
	...
	aux = (jack_default_audio_sample_t*) calloc(d,sizeof(jack_default_audio_sample_t));
...
System funciona medio raro;
las entradas son las salidas y viceversa.


Entrar a baudline
y que pueda dividir baudline y se quede ahí puesto.
