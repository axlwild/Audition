...
void shiftInTime(jack_default_audio_sample_t *buff, int buff_size, int time){
	for(int i = 0; i < buff_size; ++i){
		buff[i] *= buff[i]*cexp(-I*2* M_PI*time*buff[i]);
	}
	return;
}
...
int main (int argc, char *argv[]) {
	...
	for (int i = 0; serverports_names[i]!=NULL; ++i)
		printf("puerto salida #%d:\t%s\n",i,serverports_names[i]);
