...
double sample_rate;
// tiempo en el que va el ventaneo
int t;
int t2;
 ...
 */
double hann(int t,double period){
	return 0.5 * (1- cos(t*2*PI/(period - 1)));
}
...
int jack_callback (jack_nframes_t nframes, void *arg){
	...
	// Obteniendo el ventaneo de la función
	for(i = 0; i < nframes ; i++){
		in[i] = in[i]*hann(t, nframes) + in[i]*hann(nframes-t2,nframes);
		aux = !aux;
		if (aux != 0) {
			t++;
			t2++;
	...
	aux = (int)sampleRate/windows;
	for(i = 0; i < nframes/2; ++i){
		if ((aux*(i) < frecin) || (aux*(i)> frecfin)){
			o_fft[i]= 0;
			o_fft[nframes-i-1]= 0;
		}
		else{
			o_fft[i] = i_fft[i];
			o_fft[nframes-i-1]= i_fft[nframes-i-1];
		}
	}
	
	//o_fft[107] = 200;
	//o_fft[nframes-107] = 200;
