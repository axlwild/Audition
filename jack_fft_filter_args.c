...
void filter(jack_default_audio_sample_t *buff, int buff_size){
	int i;
	double windows = (double)jack_get_buffer_size (client);
	double sampleRate = (double)jack_get_sample_rate(client);
	double aux;
	...
	// Multiplicado por dos porque la ventana es del doble de tamaño
	aux = (sampleRate/windows)/2;

	for(i = 0; i < buff_size/2 ; ++i){
		//Menores a esta frecuencia y a su espejo
	    if(i*aux < frecmin || i*aux > frecmax){
    		o_fft[i] = 0.0;
    		o_fft[buff_size-i] = 0.0;
	    }else{
    		o_fft[i] = i_fft[i];
    		o_fft[buff_size-i] = i_fft[buff_size-i];
		}
	}
...
int main (int argc, char *argv[]) {
	...
	if (argc != 3){
		printf("Sintaxis: ./jack_fft_args frecmin frecmax\n");
		exit(-1);
	}
	if (argv[1] > argv[2]){
		printf("Frec1 must be < frec2)\n");
		exit(-1);
	}
...
/*
To compile:
gcc -std=gnu99 -o jack_fft_args jack_fft_filter_args.c -ljack -lfftw3 -lm
